            ...
            if (data.success && data.has_token && data.token) {
                ...
                tokenBox.innerHTML = `
                    <div class="card" style="border-left: 4px solid #2563eb;">
                        ...
                        <div style="font-size:36px; font-weight:800; color:#2563eb;">${t.token_code}</div>
                        <div style="font-size:16px; font-weight:600; margin-bottom:12px;">${t.service_name}</div>
                        <div style="display:flex; gap:20px; background:#f8fafc; padding:10px 14px; border-radius:6px; margin-bottom:14px;">
                            <div>
                                ...
                                <div style="font-size:18px; font-weight:700;">${data.people_ahead}</div>
                            </div>
                            <div>
                                <span style="font-size:11px; color:#64748b;">ESTIMATED WAIT</span>
                                <div style="font-size:18px; font-weight:700;">${data.estimated_wait} min</div>
                            </div>
                        </div>
                        <a href="token.html" class="btn btn-primary btn-sm">View My Token &rarr;</a>
                    </div>
                `;
            } else {
                tokenBox.innerHTML = `
...
function loadStudentToken() {
                ...
                if (data.success && data.has_token && data.token) {
                    ...
                    box.innerHTML = `
                        <div class="token-box">
                            ...
                            ${t.status === 'called' ? `
                                <div class="alert alert-success" style="font-weight:700; font-size:15px;">
    ...
    update();
    setInterval(update, 5000); // Poll every 5s
}
        ...
        .then(data => {
            ...
            if (data.current_token) {
                ...
                currentBox.innerHTML = `
                    <div style="text-align:center; padding:10px 0;">
                        ...
                        <div style="font-size:16px; font-weight:600; margin-bottom:16px;">Student: ${t.student_name}</div>
            ...
            if (data.waiting_queue && data.waiting_queue.length > 0) {
                queueList.innerHTML = `
                    <table>
                        <thead>
                            <tr><th>Token</th><th>Student</th></tr>
                        </thead>
                        <tbody>
                ...
                queueList.innerHTML = `<p style="color:#64748b; font-size:14px;">No students are waiting in queue.</p>`;
            }
        });
}
